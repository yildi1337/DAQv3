...
{
	/* Configure pins */
	TCA9534A_SetPinType(ADC_NRESET_DEVICE, ADC_NRESET_PIN, TCA9534A_PINTYPE_OUTPUT);
	TCA9534A_SetPinType(ADC_SPI_NCS_DEVICE, ADC_SPI_NCS_PIN, TCA9534A_PINTYPE_OUTPUT);

	/* Set standard voltage levels of output pins */
	TCA9534A_SetPinState(ADC_NRESET_DEVICE, ADC_NRESET_PIN, GPIO_PIN_SET);
	TCA9534A_SetPinState(ADC_SPI_NCS_DEVICE, ADC_SPI_NCS_PIN, GPIO_PIN_SET);

	/* Reset */
	AD7768_Reset();

	/* Configure ADC */
	AD7768_SPIWrite(AD7768_REGISTER_CHANNEL_STANDBY, 0x00);				/* Enable all channels */
	AD7768_SPIWrite(AD7768_REGISTER_CHANNEL_MODE_A, 0x00);				/* wideband filter, x32 decimation rate */
	AD7768_SPIWrite(AD7768_REGISTER_CHANNEL_MODE_B, 0x00);				/* wideband filter, x32 decimation rate */
	AD7768_SPIWrite(AD7768_REGISTER_CHANNEL_MODE_SELECT, 0x00);			/* channel mode A for all channels */
	AD7768_SPIWrite(AD7768_REGISTER_POWER_MODE, 0x33);					/* normal operation, fast, LVDS input clock disabled, fmod = MCLK/4 */
	AD7768_SPIWrite(AD7768_REGISTER_GENERAL_CONFIGURATION, 0x0A);		/* SYNC_OUT disabled, VCM buffer normal mode, VCM = 2.5 V */
	AD7768_SPIWrite(AD7768_REGISTER_INTERFACE_CONFIGURATION, 0x03);		/* no CRC, DCLK = MCLK, no SCLK clock divide */
...
{
	TCA9534A_SetPinState(ADC_NRESET_DEVICE, ADC_NRESET_PIN, GPIO_PIN_RESET);
...
void AD7768_ChipSelect(void)
{
	...
	TCA9534A_SetPinState(ADC_SPI_NCS_DEVICE, ADC_SPI_NCS_PIN, GPIO_PIN_RESET);
}
